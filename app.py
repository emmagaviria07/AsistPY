...
# Clase personalizada para el parsing de salida
class CustomOutputParser(AgentOutputParser):
    def parse(self, text: str) -> Union[AgentAction, AgentFinish]:
        if "--Respuesta Correcta--" in text or "Final Answer:" in text:
            # Limpia el texto de marcadores especiales
            clean_text = text.replace("--Respuesta Correcta--", "").replace("Final Answer:", "").strip()
            return AgentFinish(
...
def format_response_for_streamlit(response):
    ...
    try:
        # Si la respuesta es un diccionario, extraer el output
...
if user_question and ke and uploaded_file is not None:
    try:
        with st.spinner('Analizando los datos...'):
            ...
            # Crear el agente con el parser personalizado
            agent = create_pandas_dataframe_agent(
                ...
                output_parser=CustomOutputParser(),  # Usar el parser personalizado
